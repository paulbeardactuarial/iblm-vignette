...
This density plot will also show the standard error for the original
beta coefficient fitted in the GLM model component. This can be useful
to understanding the significance of the range in corrected beta
coefficients.

When the argument varname is set to be a continuous predictor variable a
single plot is produced. In the code below calling the function four
times, with four different varname values, produces four plots.
...
When the argument varname is set to be a categorical predictor variable,
a plot is produced for each separate level of that categorical variable.
...
beta coefficient within the GLM model component. Instead, those values
are migrated to the bias (see below).
